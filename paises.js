// ============================================================
//  paises.js — Lista completa de países con código telefónico
//  Hotel Las Quenas
// ============================================================
...
const PAISES = [
  ...
  { code: "BE", name: "Belgium / Belgique", dial: "+32" },
  ...
  { code: "EG", name: "Egypt / مصر", dial: "+20" },
...
// ── Generar el <select> de países ─────────────────────────
function generarSelectPaises(idSelect, valorDefecto = "PE") {
  const select = document.getElementById(idSelect);
  if (!select) return;

  // Países prioritarios para Perú primero
  const prioritarios = ["PE","AR","CL","BO","CO","EC","BR","US","GB","ES","DE","FR","IT","AU","JP","CN"];

  const opciones = [
    ...PAISES.filter(p => prioritarios.includes(p.code)),
    { code: "---", name: "─────────────", dial: "" },
    ...PAISES.filter(p => !prioritarios.includes(p.code)).sort((a,b) => a.name.localeCompare(b.name))
  ];

  select.innerHTML = opciones.map(p => {
    if (p.code === "---") return `<option disabled>─────────────</option>`;
    const label = `${p.name} (${p.dial})`;
    const selected = p.code === valorDefecto ? "selected" : "";
    return `<option value="${p.dial}" data-code="${p.code}" ${selected}>${label}</option>`;
  }).join("");
}

// Detectar país del usuario por idioma del navegador
function detectarPaisPorIdioma() {
  const lang = navigator.language || "es-PE";
  const mapeo = {
    "es-PE": "PE", "es-AR": "AR", "es-CL": "CL", "es-CO": "CO",
    "es-BO": "BO", "es-EC": "EC", "es-MX": "MX", "es-UY": "UY",
    "pt-BR": "BR", "pt": "PT", "en-US": "US", "en-GB": "GB",
    "en-AU": "AU", "de": "DE", "fr": "FR", "it": "IT",
    "zh": "CN", "ja": "JP", "ko": "KR"
  };
  return mapeo[lang] || mapeo[lang.substring(0,2)] || "PE";
}
